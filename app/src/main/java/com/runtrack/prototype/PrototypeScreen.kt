...
fun RunTrackPrototypeApp(viewModel: RunTrackViewModel = viewModel()) {
    ...
    val keepScreen = settings.keepScreenOn && tracking?.status == com.runtrack.prototype.domain.WorkoutStatus.ACTIVE
    ...
    LaunchedEffect(tracking?.status, selectedId) {
        if (tracking?.status == com.runtrack.prototype.domain.WorkoutStatus.FINISHING && selectedId != null) {
    ...
    MaterialTheme {
        ...
        if (tracking?.status == com.runtrack.prototype.domain.WorkoutStatus.RECOVERY_REQUIRED) {
            AlertDialog(
                onDismissRequest = {},
